...
1. Create a ticket with LF IT under "Project Support Services" <https://support.linuxfoundation.org>.
   ...
   LF IT purchases a 3-year certificate and mints a 1-year certificate for us to use. They share the `.crt` file and `_bundle.pem` (or `.ca-bundle`) file via the ticket, and share the private key via 1Password.
2. The cert and bundle file for each domain should be converted to a single `.pem` file by concatenating them, with the `.crt` file in front.
   * `cat STAR_jquery_com.crt STAR_jquery_com_bundle.pem > STAR_jquery_com.pem`
   * The first cert file must not have an EOL character, so open `STAR_jquery_com.pem` and verify sure that all block terminators look like this (i.e., not on the same line, and no blank lines in between):
     ```
     -----END CERTIFICATE-----
     -----BEGIN CERTIFICATE-----
     ```
...
4. **Test it!** by running `bin/verify_cert.sh path/to/your/star.jquery.com.pem`
   ...
   Note: If the `.key` file contains `ENCRYPTED` (that is, if `verify_cert.sh` causes openssl to prompt you for a password), then **convert this to plaintext first** via `bin/decrypt_cert_key.sh`, so that the file can be safely used by a webserver.
...
1. **Upload**. After obtaining and locally **testing** the new certificate in the above process,
   navigate the Fastly management panel to "Add a new" certificate and leave it **unused**. You will upload the `.key` and the combined `.pem` file.
2. Activate the new cert for a **secondary service** at Fastly, such as "code2" (`t.sni`).
   Confirm in our DNS that our primary sites all points to `k.sni`, with only the secondary "code2" service pointing to `t.sni`.
